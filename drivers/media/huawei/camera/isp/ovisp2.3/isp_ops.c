...
static int isp_res_init(struct device *pdev)
{
	...
	cam_notice("%s isp-base address = %pK. isp-base size = 0x%x. isp-irq = %d",
...
int hw_isp_init(struct device *pdev)
{
	...
	cam_info("%s meta_viraddr=%pK meta_phyaddr=%pK size=%u", __func__,
...
power_on_try:
	...
	cam_notice("%s isp poweron success. viraddr %pK. firmware version: 0x%x",
