...
*  return value:  0-sucess or others-fail
**********************************************************/
static int bq25892_start_force_ico(unsigned int *idpm_limit)
{
    int ret = 0;
    u8 reg = 0;
    int check_ico_optimized_times = 10;

    hwlog_info("bq25892_start_force_ico ++\n");
    ...
    if (reg & BQ25892_REG_09_FORCE_ICO_MASK) {
        hwlog_info("ICO has already forced\n" );
        return ret;
    }
    ...
    while (check_ico_optimized_times--) {
        ...
        if (reg & BQ25892_REG_14_ICO_OPTIMIZED_MASK) {
            ...
            hwlog_info("bq25892_read_byte BQ25892_REG_13 is 0x%x\n", reg);

            *idpm_limit = (unsigned int)reg * BQ25892_REG_13_IDPM_STEP_MV + BQ25892_REG_13_IDPM_OFFSET_MV;
            return 0;
        }
...
struct charge_device_ops bq25892_ops = {
    ...
    .force_ico=bq25892_start_force_ico,
};
/**********************************************************
