...
enum irda_work_state{
	IRDA_STATE_UNKNOW =- 1,  /**power status when set LDO success but set uart fail*/
	IRDA_IDLE = 0,
	IRDA_DEFAULT = 1,
};
...
static int uartpin_set(struct irda_device *irda_dev, int enable) {
	...
	pinctrl_set = pinctrl_lookup_state(irda_dev->pdata.uart_reset, enable? PINCTRL_STATE_DEFAULT : PINCTRL_STATE_IDLE);
...
static int irda_power_config(struct irda_device *irda_dev, int enable)
{
	...
	case IRDA_POWER_TYPE_INTERNAL_LDO:
		if (enable != irda_dev->pdata.power_state.power_state) {
			if (enable && (regulator_is_enabled(irda_dev->pdata.ldo_reset) <= 0)) {
		...
		if (IRDA_ERROR == ret ) {
			irda_dev->pdata.power_state.work_state = IRDA_STATE_UNKNOW;
			break;
		}
	...
	case IRDA_POWER_TYPE_EXTERNAL_LDO:
		...
		*/
		hwlog_err("do not support this type %d\n", type);
		ret = IRDA_ERROR;
		break;
	default:
...
static int irda_power_internel_ldo_init(struct device *dev,
			struct irda_device *irda_dev)
{
	...
	irda_dev->pdata.power_state.power_state = IRDA_STATE_UNKNOW;
...
*/
		...
		break;
	case IRDA_POWER_TYPE_INTERNAL_LDO:
		ret = irda_power_internel_ldo_init(dev, irda_dev);
		break;
	default:
...
static int irda_probe(struct platform_device *pdev)
{
	...
	irda_dev->pdata.power_state.uart_state = IRDA_STATE_UNKNOW;
	irda_dev->pdata.power_state.work_state = IRDA_STATE_UNKNOW;
