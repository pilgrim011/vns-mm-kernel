...
#define DMD_CASE_ALS_NEED_RECOVERY_1 23
#define DMD_CASE_ALS_NEED_RECOVERY_2 13
#define ALS_RESET_COUNT 1
#define ALS_RESET_REFRESH_PERIOD (3600*8)

static int als_retry_cnt = ALS_RESET_COUNT;
static uint64_t last_reset_period = 0;
...
int als_need_reset_power(pkt_dmd_log_report_req_t *pkt)
{
    struct timespec ts;
    int ret = 0;
    ...
    if(pkt->dmd_id == DSM_SHB_ERR_MCU_ALS
        && (pkt->dmd_case== DMD_CASE_ALS_NEED_RECOVERY_1 || pkt->dmd_case == DMD_CASE_ALS_NEED_RECOVERY_2)
    )
    {
        if((ts.tv_sec - last_reset_period) > ALS_RESET_REFRESH_PERIOD)
        {
            last_reset_period = ts.tv_sec;
...
void iom7_dmd_log_handle(struct work_struct  *work)
{
    ...
    if(als_need_reset_power(pkt))
    {
        hwlog_err("als reset sensorhub\n");
