...
void rdr_hisiap_print_all_dump_addrs(void)
{
	...
	printk(KERN_INFO "[%s], buddy_fail_addr [0x%pK].\n", __func__,
...
{
	AP_RECORD_ETR *petr;
	printk(KERN_SOH "[%s]:enter g_bbox_ap_record_pc=%pK\n",
...
int __init rdr_hisiap_init(void)
{
	...
	if (REBOOT_REASON_LABEL3 <= reboot_type
		...
		recordTask = kthread_run(save_exception_info, "thread",
				"save_exception");
		printk(KERN_INFO
		       "%s: create save_exception_info, return %pK\n",
		       __func__, recordTask);
	}
	...
	if (CP_S_MODEMAP == reboot_type
		...
		recordTask = kthread_run(save_kernel_dump, "thread",
				"save_kerneldump");
		printk(KERN_INFO
		       "%s: create save_kernel_dump, return %pK\n",
		       __func__, recordTask);
	}
