...
#define HI64xx_TRY_MS_PER_CYCLE        40
#define HI64xx_GET_LOCK_TIMEOUT_LEN    800
...
static bool hi64xx_mutex_trylock_timeout(struct mutex *lock, unsigned int time_len_ms)
{
	int count = 0;

	if (time_len_ms <= HI64xx_TRY_MS_PER_CYCLE)
		count = 1;
	else
		count = time_len_ms/HI64xx_TRY_MS_PER_CYCLE;

	while (count) {
		if (mutex_trylock(lock))
			return true;

		msleep(HI64xx_TRY_MS_PER_CYCLE);
		count--;
	}
...
static void hi64xx_mutex_unlock(struct mutex *lock)
{
	mutex_unlock(lock);
}
...
static int hi64xx_sync_write(const unsigned char *arg, const unsigned int len)
{
	...
	if (!hi64xx_mutex_trylock_timeout(&dsp_priv->state_mutex, HI64xx_GET_LOCK_TIMEOUT_LEN)) {
		HI64XX_DSP_ERROR("get mutex lock fail\n");
		OUT_FUNCTION;
		return -BUSY;
	}
...
out:
	hi64xx_mutex_unlock(&dsp_priv->state_mutex);
	OUT_FUNCTION;
...
static void hi64xx_hifi_pause(void)
{
	...
	if (0 == wait_cycle) {
		HI64XX_DSP_ERROR("wait dsp wfi timeout, dsp_status0:0x%x, wfi state:0x%x(0:enter,1:exit,0xff:not hi6402)\n",
			hi64xx_hifi_read_reg(HI64xx_DSP_DSP_STATUS0),
			(dsp_priv->dsp_config.wfi_state_addr != 0) ? hi64xx_hifi_read_reg(dsp_priv->dsp_config.wfi_state_addr) : 0xff);
	}
...
static void hi64xx_dsp_stop(void)
{
	...
	if (0 == wait_cycle) {
		HI64XX_DSP_ERROR("wait dsp wfi timeout, dsp_status0:0x%x\n",
			hi64xx_hifi_read_reg(HI64xx_DSP_DSP_STATUS0));
	}
...
static int hi64xx_func_fw_download(struct krn_param_io_buf *param)
{
	...
	if (dsp_priv->dsp_config.run_state_addr)
		hi64xx_hifi_write_reg(dsp_priv->dsp_config.run_state_addr, 0);
	...
	if (wait_event_interruptible_timeout(dsp_priv->dsp_pwron_wq,
		...
		if (dsp_priv->dsp_config.run_state_addr)
			HI64XX_DSP_ERROR("dsp running state:0x%x (running:0x5a5a else 0)\n",
				hi64xx_hifi_read_reg(dsp_priv->dsp_config.run_state_addr));
		hi64xx_dump_debug_info();

		/* can't get codec version,reset system */
		BUG_ON(hi64xx_error_detect());
		/* after retry 3 times, reset system */
		BUG_ON(reload_retry_count == RELOAD_RETRY_MAX);

		if (!(dsp_priv->is_watchdog_coming) && !(dsp_priv->is_sync_write_timeout)
			&& reload_retry_count < RELOAD_RETRY_MAX) {
			dsp_priv->is_sync_write_timeout = true;
			reload_retry_count++;
			HI64XX_DSP_ERROR("do reset codechifi,retry %d\n", reload_retry_count);
#if !defined(PLATFORM_HI3660)
			rdr_codec_hifi_watchdog_process();
#endif
		}
