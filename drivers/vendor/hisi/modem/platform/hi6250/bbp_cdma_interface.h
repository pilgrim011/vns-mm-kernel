 ...
 ******************************************************************************
  ...
  1.日    期   : 2015年6月12日
    作    者   : l00165286
    修改内容   : 从《Hi6250V100 MODEM寄存器手册_CDMA.xml》自动生成
...
typedef union
{
    ...
    struct
    {
        unsigned int  rfic_start_time : 11; /* bit[0-10] : rfic指令的生效时刻，该域当S配置为0时有效，其配置内容为系统时标对应的位置，定时模块需要根据软件的配置选取当前工作的系统时标输出给前端控制模块，精度为chip，CBBP根据配置内容与定时模块输出的绝对时标进行比较，其存在二种情况：
                                                           二者相等或配置绝对时标小于定时绝对时标时，则读取数据Buffer内容，并发送；
                                                           配置绝对时标不等于定时绝对时标，则继续等待。 */
        unsigned int  reserved_0      : 5;  /* bit[11-15]: 保留 */
...
typedef union
{
    ...
    struct
    {
        unsigned int  mipi_start_time : 11; /* bit[0-10] : mipi指令生效时刻，该域当S配置为0时有效，其配置内容为系统时标对应的位置，定时模块需要根据软件的配置选取当前工作的系统时标输出给前端控制模块，精度为chip，CBBP根据配置内容与定时模块输出的绝对时标进行比较，其存在二种情况：
                                                           二者相等或配置绝对时标小于定时绝对时标时，则读取数据Buffer内容，并发送；
                                                           配置绝对时标不等于定时绝对时标，则继续等待。 */
        unsigned int  reserved_0      : 5;  /* bit[11-15]: 保留 */
...
typedef union
{
    ...
    struct
    {
        unsigned int  gpio_start_time : 11; /* bit[0-10] : GPIO指令的生效时刻，该域当S配置为0时有效，其配置内容为系统时标对应的位置，定时模块需要根据软件的配置选取当前工作的系统时标输出给前端控制模块，精度为chip，CBBP根据配置内容与定时模块输出的绝对时标进行比较，其存在二种情况：
                                                           二者相等或配置绝对时标小于定时绝对时标时，则读取数据Buffer内容，并发送；
                                                           配置绝对时标不等于定时绝对时标，则继续等待。 */
        unsigned int  reserved_0      : 5;  /* bit[11-15]: 保留 */
...
typedef union
{
    ...
    struct
    {
        unsigned int  hkadc_start_time : 11; /* bit[0-10] : hkadc指令的生效时刻，该域当S配置为0时有效，其配置内容为系统时标对应的位置，定时模块需要根据软件的配置选取当前工作的系统时标输出给前端控制模块，精度为chip，CBBP根据配置内容与定时模块输出的绝对时标进行比较，其存在二种情况：
                                                            二者相等或配置绝对时标小于定时绝对时标时，则读取数据Buffer内容，并发送；
                                                            配置绝对时标不等于定时绝对时标，则继续等待。 */
        unsigned int  reserved_0       : 5;  /* bit[11-15]: 保留 */
