...
)
{
    ...
    if ((USIMM_SW_WARNING == ulResult) || (USIMM_SW_TECH_ERROR == ulResult))
    {
        if (1 == ulSendCnt) /*第一次发送*/
        {
            ...
            else if(USIMM_SELECT_ACTIVATE_AID == pstApduInfo->aucAPDU[P2])
            {
            ...
            {
                USIMM_WARNING_LOG("USIMM_SelectFile_APDU:USIMM_SENDAPDU_FAIL");

                return USIMM_SENDAPDU_FAIL;
            }

            USIMM_WARNING_LOG("USIMM_SelectFile_APDU:USIMM_SENDAPDU_RETRY");
...
VOS_VOID USIMM_AttApduSWCheck(VOS_VOID)
{
    if (VOS_TRUE == g_stUsimmFeatureCfg.unCfg.stFeatureCfg.ulAtt_flg)
    {
        ...
        {
            USIMM_WARNING_LOG("USIMM_AttApduSWCheck: ATT Need IND No Card");
...
)
{
    ...
    {
        /*lint -e534*/
        VOS_MemCpy(pstRspData->aucRsp, gstUSIMMAPDU.aucRecvBuf, gstUSIMMAPDU.ulRecDataLen);
        /*lint +e534*/
    }
...
VOS_UINT32 USIMM_DLResetCard(VOS_UINT32 ulResetType, VOS_INT32 *plVoltageSwitchRst)
{
    ...
    else
    {
        ...
        *plVoltageSwitchRst = lSCIResult;
        ...
        else                     /*电压切换失败按照无卡处理*/
        {
            ...
            USIMM_ERROR_LOG("USIMM_DLResetCard: mdrv_sci_switch_class Error Set Nocard");

#if (FEATURE_ON == FEATURE_PTM)
        USIMM_RecordCardErrorLog(USIMM_MNTN_MODULE_INIT,
                                USIMM_MNTN_INIT_FAIL, 
                                USIMM_MNTN_INIT_RESETERR,
                                VOS_NULL,
    ...
    {
        USIMM_ERROR_LOG("USIMM_DLResetCard: mdrv_sci_get_cardstatus Could Not Find Any SIM Card!");
    ...
    USIMM_APDU_RSP_STRU                 *pstRspData)
{
    ...
    if (VOS_OK != ulResult)
    {
        USIMM_WARNING_LOG("USIMM_SendStatusPollingApdu:USIMM_SendAPDUHandle error");
    }
    ...
    VOS_UINT8                           ucLen)
{
    ...
    if (VOS_OK != ulResult)
    {
        USIMM_WARNING_LOG("USIMM_SendUpdateRecordApdu:USIMM_SendAPDUHandle error");
    }
...
)
{
    ...
    for (usUpdateLen=0; usUpdateLen<usLen; usUpdateLen += ucP3)
    {
        ...
        {
            USIMM_WARNING_LOG("USIMM_SendUpdateBinaryApdu:USIMM_SendAPDUHandle error");
    ...
    VOS_UINT8                          *pucContent)
{
    ...
    for(; i<=ulTotalNum; i++)
    {
        ...
        if(VOS_OK != USIMM_SendAPDUHandle(USIMM_APDUCMD_COMMON,
                                            ...
                                            &stRspData))/*判断发送结果*/
        {
            USIMM_WARNING_LOG("USIMM_SendReadRecordApdu: USIMM_SendAPDUHandle Error");
    ...
    VOS_UINT8                           *pucContent)
{
    ...
    for (usReadedLen=0; usReadedLen<usNeedLen; usReadedLen+=ucP3)
    {
        ...
        {
            USIMM_WARNING_LOG("USIMM_SendReadBinaryApdu: USIMM_SendAPDUHandle Error");
    ...
    USIMM_TERMINAL_CAPABILITY_STRU     *pstTerminalCapability)
{
    ...
    if (VOS_OK != ulResult)
    {
        USIMM_WARNING_LOG("USIMM_SendTerminalCapabilityApdu:USIMM_SendAPDUHandle error");
    }
    ...
    VOS_UINT8                          *pucData)
{
    ...
    if (VOS_OK != ulResult)
    {
        USIMM_WARNING_LOG("USIMM_SendConfirmSsdApdu:USIMM_SendAPDUHandle error");
    }
...
)
{
    ...
    if (VOS_OK != ulResult)
    {
        USIMM_WARNING_LOG("USIMM_SendStoreESNMEIDApdu:USIMM_SendAPDUHandle error");
    }
    ...
    USIMM_APDU_RSP_STRU                *pstRspData)
{
    ...
    if (VOS_OK != ulResult)
    {
        USIMM_WARNING_LOG("USIMM_SendComputeIPApdu:USIMM_SendAPDUHandle error");
    }
    ...
    USIMM_APDU_RSP_STRU                *pstRspData)
{
    ...
    if (VOS_OK != ulResult)
    {
        USIMM_WARNING_LOG("USIMM_SendBCMCSApdu:USIMM_SendAPDUHandle error");
    }
    ...
    USIMM_APDU_RSP_STRU                *pstRspData)
{
    ...
    if (VOS_OK != ulResult)
    {
        USIMM_WARNING_LOG("USIMM_SendAppAuthApdu:USIMM_SendAPDUHandle error");
    }
