...
VOS_UINT32 USIMM_STATUSProc(VOS_VOID)
{
    ...
    USIMM_DFFCP_ST                      *pstCurDFFile;

    ulResult = USIMM_SendStatusPollingApdu(&stRspData);

    if (USIMM_SW_OK != ulResult)
    {
        USIMM_WARNING_LOG("USIMM_STATUSProc: Command SW is Error");

        return ulResult;
    }

    USIMM_INFO_LOG("USIMM_STATUSProc: The Status Command is OK");

    pstCurFile = USIMM_CCB_GetCurFileInfo(USIMM_UNLIMIT_AUTO);

    if (VOS_NULL_PTR == pstCurFile)
    {
        USIMM_WARNING_LOG("USIMM_STATUSProc: USIMM_CCB_GetCurFileInfo Fail");

        return USIMM_SW_OK;
    }

    /* 根据当前主应用类型得到当前的DF和DF的FCP */
    usCurDfID   = pstCurFile->usCurDFID;
    usCurFcpLen = pstCurFile->usCurDFFcpLen;

    ulResult = USIMM_DecodeDFFcp(USIMM_CCB_GetCardType(), &stRspData, pstCurFile);

    if (ulResult != VOS_OK)
    {
        /* 出错需要还原之前的值，避免下次发送错误 */
        pstCurFile->usCurDFFcpLen  = usCurFcpLen;

        pstCurFile->usCurDFID      = usCurDfID;
    ...
    pstCurDFFile = USIMM_CCB_GetCurDFInfo(USIMM_UNLIMIT_AUTO);

    if (VOS_NULL_PTR == pstCurDFFile)
    {
        USIMM_WARNING_LOG("USIMM_STATUSProc: USIMM_CCB_GetCurDFInfo Fail");
    ...
    if (USIMM_PHYCARD_TYPE_ICC == USIMM_CCB_GetCardType())  /* 当前为SIM */
    {
        ...
        pstCurFile->usCurDFID = pstCurDFFile->usDFID;
    }
    else
    {
        ...
        if (VOS_OK == VOS_MemCmp(pstUiccAdfInfo->aucAID,
                                  pstCurDFFile->aucName,
                                  pstUiccAdfInfo->ulAIDLen))
        {
            pstCurFile->usCurDFID = ADF;
        /*lint +e534*/
        }
        else
        {
    ...
    usCurDfIDTemp   = pstCurFile->usCurDFID;

    if ((USIMM_NULL_ID != usCurDfID) && (usCurDfID != usCurDfIDTemp))
    {
        USIMM_ERROR_LOG("USIMM_STATUSProc:The Status Command is not Same,The Card Should be Changed");
...
)
{
    ...
    if (USIMM_PHYCARD_TYPE_UICC == USIMM_CCB_GetCardType())
    {
        ...
        {
            USIMM_WARNING_LOG("USIMM_UnblockPIN: Get PUK Remain Time fail");
        }
    }
    else
    {
...
)
{
    ...
    if (VOS_NULL_PTR == pstPinInfo)
    {
        USIMM_ERROR_LOG("USIMM_PINVerify: enAppType error");
...
VOS_UINT32 USIMM_InitUsimFDNBDNStatus(VOS_VOID)
{
    ...
    {
        USIMM_ERROR_LOG("USIMM_InitUsimFDNBDNStatus: Select EFEST Error");
    }
    ...
    USIMM_FDNSTATUS_ENUM_UINT32         *pulFdnStatus)
{
    ...
    if (PS_USIM_SERVICE_NOT_AVAILIABLE == USIMM_IsServiceAvailable(SIM_SVR_FDN))
    {
        *pulFdnStatus = USIMM_FDNSTATUS_OFF;
        ...
        USIMM_WARNING_LOG("USIMM_UICCFDNEnable: FDN SVR off");
    ...
    if (PS_USIM_SERVICE_NOT_AVAILIABLE == USIMM_IsServiceAvailable(SIM_SVR_ADN))
    {
        *pulFdnStatus = USIMM_FDNSTATUS_ON;
        ...
        USIMM_WARNING_LOG("USIMM_UICCFDNEnable: ADN SVR off");
    ...
    else
    {
        ...
        if (USIMM_EFSTATUS_DEACTIVE == USIMM_CCB_GetCurEFInfo(USIMM_GUTL_APP)->enFileStatus)
        {
            *pulFdnStatus = USIMM_FDNSTATUS_ON;
            ...
            USIMM_WARNING_LOG("USIMM_UICCFDNEnable: ADN is DEACTIVE");
        }
        /* ADN文件有效,FDN未激活 */
        else
        {
    ...
    USIMM_BDNSTATUS_ENUM_UINT32         *pulBdnStatus)
{
    ...
    if (PS_USIM_SERVICE_NOT_AVAILIABLE == USIMM_IsServiceAvailable(SIM_SVR_CALL_CONTROL))
    {
        *pulBdnStatus = USIMM_BDNSTATUS_OFF;
        ...
        USIMM_WARNING_LOG("USIMM_UICCFDNEnable: Not support call control");
    ...
    if (PS_USIM_SERVICE_NOT_AVAILIABLE == USIMM_IsServiceAvailable(SIM_SVR_BDN))
    {
        *pulBdnStatus = USIMM_BDNSTATUS_OFF;
        ...
        USIMM_WARNING_LOG("USIMM_UICCFDNEnable: BDN SVR off");
    ...
    if (VOS_OK != ulResult)
    {
        *pulBdnStatus = USIMM_BDNSTATUS_OFF;
        ...
        USIMM_WARNING_LOG("USIMM_UICCFDNEnable: Select EFBDN Error");
    }
    else
    {
        /* BDN文件有效,BDN激活 */
        if (USIMM_EFSTATUS_ACTIVE == USIMM_CCB_GetCurEFInfo(USIMM_GUTL_APP)->enFileStatus)
        {
            *pulBdnStatus = USIMM_BDNSTATUS_ON;
            ...
            USIMM_WARNING_LOG("USIMM_UICCFDNEnable: EFBDN is active");
        }
        /* BDN文件无效,BDN未激活 */
        else
        {
    ...
    SCI_ATRINFO_S                       *pstSCIATRInfo)
{
        ...
        && (T1_PROTOCOL_MODE == pstSCIATRInfo->Tbyte))
    {
        ...
        if ((VOS_OK != ulResult) || (VOS_OK != ulResult2))
        {
            ...
            if(VOS_OK != mdrv_sci_switch_protocol((PROTOCOL_MODE_E)g_enUSIMMTMode))
            {
                USIMM_ERROR_LOG("USIMM_DecideTMode:mdrv_sci_switch_protocol fail");
    ...
    USIMM_DEF_FILEID_ENUM_UINT32        enUsimID)
{
    ...
    /* 找不到则返回空 */
    USIMM_WARNING_LOG("USIMM_UsimEFIDToIsimEFID: Could not find File ID");
    ...
    USIMM_DEF_FILEID_ENUM_UINT32        enIsimID)
{
    ...
    /* 找不到则返回空 */
    USIMM_WARNING_LOG("USIMM_IsimEFIDToUsimEFID: Could not find file ID");
    ...
    UICC_SERVICES_TYPE_ENUM_UINT32      enService)
{
    ...
    {
        USIMM_WARNING_LOG("USIMM_IsSIMServiceAvailable: USIMM_GUTL_APP status is Wrong");
    ...
    {
        USIMM_WARNING_LOG("USIMM_IsServiceAvailable: USIMM_PoolFindFile SST is Failed");
    ...
    UICC_SERVICES_TYPE_ENUM_UINT32      enService)
{
    ...
    {
        USIMM_WARNING_LOG("USIMM_IsUSIMServiceAvailable: The Card Type or status is Wrong");
    ...
    if(ulResult != USIMM_API_SUCCESS)
    {
        USIMM_WARNING_LOG("USIMM_IsUSIMServiceAvailable: USIMM_PoolFindFile EFUST is Failed");
    ...
    UICC_SERVICES_TYPE_ENUM_UINT32      enService)
{
    ...
    {
        USIMM_WARNING_LOG("USIMM_IsISIMServiceAvailable: The USIMM_IMS_APP status is Wrong");
    ...
    ulResult = USIMM_PoolFindFile(EFISIMIST, &ulFileNum, USIMM_IMS_APP);
     ...
     if (ulResult != USIMM_API_SUCCESS)
     {
         USIMM_WARNING_LOG("USIMM_IsISIMServiceAvailable: USIMM_PoolFindFile EFISIMIST is Failed");
...
)
{
    ...
    {
        USIMM_WARNING_LOG("USIMM_IsUIMServiceAvailable: The USIMM_CDMA_APP status is Wrong");
    ...
    if(ulResult != USIMM_API_SUCCESS)
    {
        USIMM_WARNING_LOG("USIMM_IsUIMServiceAvailable: USIMM_PoolFindFile EFCST is Failed");
...
)
{
    ...
    if (USIMM_CARD_SERVIC_AVAILABLE != USIMM_CCB_GetAppService(USIMM_CDMA_APP))
    {
        USIMM_WARNING_LOG("USIMM_IsCSIMServiceAvailable: The USIMM_CDMA_APP status is Wrong");
    ...
    if(ulResult != USIMM_API_SUCCESS)
    {
        USIMM_WARNING_LOG("USIMM_IsCSIMServiceAvailable: USIMM_PoolFindFile EFCSIMST is Failed");
...
)
{
    ...
    switch(pstEFInfo->enFileType)
    {
        ...
        case USIMM_EFSTRUCTURE_CYCLE:
            ...
            {
                USIMM_WARNING_LOG("USIMM_GetReadFilePara: The Record Index is Out of the File Range");
...
VOS_VOID USIMM_CardStatusRegCbFuncProc(VOS_VOID)
{
    ...
    {
        USIMM_WARNING_LOG("USIMM_CardStatusRegCbFuncProc: The NV en_NV_Item_Card_Status_Cb_Type is Wrong");
        return;
    }
...
VOS_VOID USIMM_SetImsiFile(VOS_VOID)
{
    ...
    if (VOS_ERR == ulResult)
    {
        USIMM_ERROR_LOG("USIMM_SetImsiFile: USIMM_PoolFindFile EFIMSI fail");/*打印错误*/
    ...
    if (VOS_NULL_PTR == USIMM_CCB_GetPoolFileByIndex(ulRecordNum)->pucContent)
    {
        USIMM_ERROR_LOG("USIMM_SetImsiFile: EFIMSI Content is Empty");/*打印错误*/
...
)
{
    ...
    {
        USIMM_ERROR_LOG("USIMM_SelectFileByDefFileId: USIMM_ChangeDefFileToPath err");
        return VOS_ERR;
    }
    ...
    USIMM_U8_DATA_STRU                  *pstCmdData)
{
    ...
    {
        USIMM_WARNING_LOG("USIMM_MakeRetrieveSKData: Data Len is error");
    ...
    USIMM_U8_DATA_STRU                  *pstCmdData)
{
    ...
    {
        USIMM_WARNING_LOG("USIMM_MakeUpdateBAKData: Data Len is error");
    ...
    USIMM_U8_DATA_STRU                  *pstCmdData)
{
    ...
    {
        USIMM_WARNING_LOG("USIMM_MakeDeleteBAKData: Data Len is error");
    ...
    USIMM_U8_DATA_STRU                  *pstCmdData)
{
    ...
    {
        USIMM_WARNING_LOG("USIMM_MakeReSRTPData: Data Len is error");
    ...
    USIMM_U8_DATA_STRU                  *pstCmdData)
{
    ...
    {
        USIMM_WARNING_LOG("USIMM_MakeGenAuthData: Data Len is error");
    ...
    USIMM_U8_DATA_STRU                  *pstCmdData)
{
    ...
    {
        USIMM_WARNING_LOG("USIMM_MakeBCMCSAuthData: Data Len is error");
        return USIMM_SW_CMD_ERROR;
    }
    ...
    USIMM_BCMCS_DATA_STRU               *pstBCMCS)
{
    ...
    switch(pstBCMCS->enBCMCSType)
    {
        ...
        default:
            USIMM_WARNING_LOG("USIMM_CdmaSpecAuthBCMCS: enBCMCSType is error");
            ulResult = USIMM_SW_CMD_ERROR;
            break;
    }
...
VOS_VOID USIMM_CheckTimeOutError(VOS_VOID)
{
    ...
    {
        USIMM_WARNING_LOG("USIMM_CheckTimeOutError: SCI TimeOut");
...
VOS_BOOL USIMM_CheckSupportAP(VOS_VOID)
{
    ...
    {
        USIMM_WARNING_LOG("USIMM_CheckSupportAP: Read en_NV_Item_System_APP_Config Failed");
    ...
    {
        USIMM_NORMAL_LOG("USIMM_CheckSupportAP: System App is Android");

        return VOS_TRUE;
    }

    USIMM_NORMAL_LOG("USIMM_CheckSupportAP: System App is other");
