    ...
    USIMM_TELECOM_AUTH_INFO_STRU       *pstCnfInfo)
{
    ...
    if ((USIMM_AUTH_UMTS_OTHER_FAILURE == pstCnfInfo->enResult)
        ||(USIMM_AUTH_GSM_OTHER_FAILURE== pstCnfInfo->enResult)
        ||(USIMM_AUTH_MAC_FAILURE      == pstCnfInfo->enResult))
    {
            USIMM_RecordCardErrorLog(USIMM_MNTN_MODULE_STATE,
                                    USIMM_MNTN_GUTL_AUTH_FAIL, 
                                    (VOS_UINT8)pstMsg->enAuthType,
                                    (VOS_UINT16)pstCnfInfo->enResult,
...
#if (FEATURE_ON == FEATURE_PTM)
    if ((USIMM_AUTH_IMS_OTHER_FAILURE == pstCnfInfo->enResult)
        ||(USIMM_AUTH_MAC_FAILURE      == pstCnfInfo->enResult))
    {
            USIMM_RecordCardErrorLog(USIMM_MNTN_MODULE_STATE,
...
#if (FEATURE_ON == FEATURE_PTM)
    if ((USIMM_AUTH_CDMA_OTHER_FAILURE == pstCnfInfo->enResult)
        ||(USIMM_AUTH_MAC_FAILURE      == pstCnfInfo->enResult))
    {
            USIMM_RecordCardErrorLog(USIMM_MNTN_MODULE_STATE,
