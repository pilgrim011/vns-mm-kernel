...
/* API for Activating the Policy with modified(Updated) Proposal & SPI & Algorithms (Smooth Switch Over)*/
extern ULONG TCPIP_IPSEC_ActivateModPolicy(UCHAR *szMapName, ULONG ulMapSeqNo);

/* IPv4 API for Deleting SA based on Inbound & OutBound SPI once New proposal is configured*/
extern ULONG TCPIP_IPSEC_DelSABySPI (ULONG ulInboundSpi, ULONG ulOutboundSPI, 
                                        ULONG ulDestAddr, UCHAR ucProto);

/* IPv6 API for Deleting SA based on Inbound & OutBound SPI, once New proposal is configured*/
extern ULONG TCPIP_IPSEC6_DelSABySPI (ULONG ulInboundSpi, ULONG ulOutboundSPI, 
                                        IN6ADDR_S *pstIPv6DestAddr, UCHAR ucProto);

/* IPv6 API for Deleting SA based on Inbound & OutBound SPI, once New proposal is configured*/
extern ULONG TCPIP_IPSEC6_DelSABySPI_str (ULONG ulInboundSpi, ULONG ulOutboundSPI, 
                                        CHAR *pucIpv6Address, UCHAR ucProto);

/* Registered function for Callback API for capturing packet before Encryption & after Decryption */
ULONG TCPIP_IPSEC_GetIPPkt_RegCallbackAPI(IPSEC_GET_IPPKT_CALLBACK_S *pstPPICallbackAPI);
ULONG TCPIP_IPSEC6_GetIPPkt_RegCallbackAPI(IPSEC6_GET_IPPKT_CALLBACK_S *pstPPICallbackAPI);

/* API to check packet is encapsulated or decapsulated  */
ULONG TCPIP_IPSEC_IsAllowedCapturePkt(MBUF_S *pMBuf);
