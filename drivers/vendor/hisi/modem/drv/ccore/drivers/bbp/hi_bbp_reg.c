...
#ifdef BBP_DMA_FIFO_BUG
void hi_bbpdma_chn_soft_rst(void)
{
    unsigned int  srst_status = 0;
    /*dma chn srst*/
    bbp_bit_set(g_bbpinfo.part[BBP_DMA].reg_base + 0x0, (u32)0x1 << 2);
    do{
        srst_status = readl(g_bbpinfo.part[BBP_DMA].reg_base + 0x10);
    }while(0 == (srst_status & (1 << 2)));

    bbp_bit_clr(g_bbpinfo.part[BBP_DMA].reg_base + 0x0, (u32)0x1 << 2);
