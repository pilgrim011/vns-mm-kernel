												...
												CIPHER_SINGLE_CFG_S * pstCfg)
{
	...
	chn_info = &(cipher_dbg_log.chn_info[chn_id]);
...
int mdrv_cipher_enable(void)
{
	unsigned long flag = 0;
	int limit = 100;
	struct cipher_ctl *ctrl = &cipher_module_ctl;

	spin_lock_irqsave(&(ctrl->clk_lock), flag);
	
	if(clk_is_enabled(ctrl->pclk)) {
		spin_unlock_irqrestore(&(ctrl->clk_lock), flag);		
		return MDRV_OK;
	}
	
	if(clk_enable(ctrl->pclk))
	{
		spin_unlock_irqrestore(&(ctrl->clk_lock), flag);
		return MDRV_ERROR;
	}
	...
	while (limit--) {
		if (clk_is_enabled(ctrl->pclk)) {
			cipher_dbg_log.clk_en = 1;	
			break;
		}
	}
	if (limit < 0) {
		cipher_dbg_log.enable_clk_failed++;
		spin_unlock_irqrestore(&(ctrl->clk_lock), flag);
		return CIPHER_CHECK_ERROR;
	}
...
int  mdrv_cipher_disable(unsigned int timeout)
{
	...
	if(!clk_is_enabled(ctrl->pclk)) {	//check cipher clock again
		spin_unlock_irqrestore(&(ctrl->clk_lock), flag);
		return MDRV_OK;
	}
	...
	clk_disable(ctrl->pclk);	
	cipher_dbg_log.clk_en = 0;
...
int cipher_resume(struct dpm_device *dev)
{
	...
	for(i = 0; i < cipher_module_ctl.chn_cnt; i++)
	{
		...
		cur_chn = &cipher_module_ctl.cipher_chn[i];
		chn_info = &(cipher_dbg_log.chn_info[i]);

		/* Clear normal channel's record */
		(void)(*cur_chn->bd_fifo.ops.rst_rwidx)(&(cur_chn->bd_fifo));
		(void)(*cur_chn->rd_fifo.ops.rst_rwidx)(&(cur_chn->rd_fifo));
		(void)(*cur_chn->cd_fifo.ops.rst_rwidx)(&(cur_chn->cd_fifo));
		cur_chn->pre_check_bd_idx = 0;
		chn_info->cfg_bd_num = 0;
		chn_info->get_rd_num = 0;
	...
	kdf_ctl.cfg_bd_num = 0;
	kdf_ctl.get_rd_num = 0;
