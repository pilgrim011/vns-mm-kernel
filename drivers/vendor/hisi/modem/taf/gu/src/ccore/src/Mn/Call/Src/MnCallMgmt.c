...
)
{
     ...
     && (VOS_TRUE == ulCallRedialSupportFlg))
    {
        ...
        if (ulCallRedialPeriodRemainLen > 0)
        {
            ...
            /*只在仅有一路呼叫的情况下处理重试，不影响之前eCall的流程*/
...
/*****************************************************************************
 函 数 名  : TAF_CALL_GetDisconnectDir
 功能描述  : 获取当前CALL ID挂断的方向
 输入参数  : 无
 输出参数  : 无
 返 回 值  :
 调用函数  :
...
/*****************************************************************************
 函 数 名  : TAF_CALL_IsCallRedialCause_Disc
 功能描述  : 原因值是否需要重拨(DISCONNECT)
 输入参数  : NAS_CC_CAUSE_VALUE_ENUM_U32:呼叫失败原因值
 输出参数  : 无
 返 回 值  : VOS_TRUE: 需要重拨
                        VOS_FALSE:不需要重拨
