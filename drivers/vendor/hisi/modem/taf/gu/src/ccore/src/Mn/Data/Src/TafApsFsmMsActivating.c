...
/*****************************************************************************
 ...
 修改历史      :
  ...
  2.日    期   : 2015年01月28日
    作    者   : w00316404
    修改内容   : send pdp activate ind to ND Client
*****************************************************************************/
...
/*****************************************************************************
 ...
 修改历史      :
  ...
  4.日    期   : 2016年03月03日
    作    者   : w00316404
    修改内容   : 发送pdp activate ind到ND Client移到ESM CNF函数中
*****************************************************************************/
...
/*****************************************************************************
 ...
 修改历史      :
  ...
  6.日    期   : 2016年7月2日
    作    者   : w00316404
    修改内容   : DTS2016062907860: IMSA发起的IMS承载激活请求需要在Attach默认承载激活之后再发起
*****************************************************************************/
VOS_VOID TAF_APS_RcvAtPsCallOrigReq_MsActivating_LteMode(VOS_VOID)
{
    ...
    TAF_PS_MSG_STRU                    *pstAppMsg;
    TAF_PS_CALL_ORIG_REQ_STRU          *pstCallOrigReq;
    VOS_UINT32                          ulAttachBearExist;
    VOS_UINT32                          ulAnyBearExist;
    VOS_UINT32                          ulAttachBearReestFlg;
    VOS_UINT32                          ulTimerLen;
    TAF_APS_TIMER_STATUS_ENUM_U8        enTimerStatus;
    VOS_UINT8                           ucAttachAllowFlg;
    VOS_UINT8                           ucPdpId;
    ...
    /* 如果注册承载不存在且注册允许标识为TRUE, 则缓存该呼叫建立请求;
       或者如果注册承载不存在且注册承载重建特性配置项为TRUE, 则缓存该呼叫建立请求	*/
    ulAnyBearExist       = TAF_APS_IsAnyBearerExist();
    ucAttachAllowFlg     = TAF_APS_GetAttachAllowFlg();
    ulAttachBearExist    = TAF_APS_IsAttachBearerExist();
    ulAttachBearReestFlg = TAF_APS_GetAttachBearerReestFlg();
    if ( ((VOS_FALSE == ulAnyBearExist) && (VOS_TRUE == ucAttachAllowFlg))
      || ((VOS_FALSE == ulAttachBearExist) && (VOS_TRUE == ulAttachBearReestFlg)) )
    {
        TAF_APS_SetPdpEntAddrType(ucPdpId, pstCallOrigReq->stDialParaInfo.enPdpType);
        TAF_APS_SetCurrPdpEntitySubFsmState(TAF_APS_MS_ACTIVATING_SUBSTA_SUSPEND);
        enTimerStatus = TAF_APS_GetTimerStatus(TI_TAF_APS_MS_ACTIVATING, ucPdpId);
