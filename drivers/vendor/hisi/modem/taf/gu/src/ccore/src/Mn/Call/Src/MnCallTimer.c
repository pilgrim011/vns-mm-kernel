...
MN_CALL_TIMER_INFO_STRU                  g_astCallTimerInfoTbl[MN_CALL_TID_BUTT - MN_TIMER_CLASS_CCA] =
{
    ...
    /* 将重试的间隔默认时间改为2s，目的是为了防止在等待间隔中下重选回4G,这样目前总时间30s，间隔时间2s*/
    {28000, MN_CALL_CallRedialPeriodTimeout},
    {2000,  MN_CALL_CallRedialIntervalTimeout},
...
VOS_VOID MN_CALL_CallRedialPeriodTimeout(VOS_UINT32 ulParam)
{
    ...
    /*超时可能发生在最后一次重试前或重试时*/
    /*如果当前有呼叫正在进行，则通知底层允许FR，否则不通知*/
    if ((VOS_TRUE == pstMsgBuff->bitOpBufferedSetupMsg))
    {
         if (MN_CALL_TIMER_STATUS_STOP == TAF_CALL_GetRedialIntervalTimerStatus(pstMsgBuff->stBufferedSetupMsg.ucCallId))
         {
            TAF_CALL_SendCcCallStatusNty(pstMsgBuff->stBufferedSetupMsg.ucCallId, MNCC_CALL_STATUS_LAST_REDIAL);
         }
    }
