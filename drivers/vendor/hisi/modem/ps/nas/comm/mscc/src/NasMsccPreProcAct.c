...
5.日    期   : 2015年10月14日
  ...
  7.日    期   : 2015年12月21日
    作    者   : j00174725
    修改内容   : DTS2015121106874  启动TI_NAS_MSCC_WAIT_IMSA_IMS_VOICE_CAP_NOTIFY定时器，由注册状态移到服务状态通知IMS时.
                 并且修改为无条件启动。
*****************************************************************************/
...
)
{
    ...
    /* 通知MMC当前IMS voice是否可用 */
    /* 如果是关机流程，则不给MMC发ImsVoiceCapInd，因为在EPS ONLY下，会disable lte，就可能导致关机时不发detach */
    pstCurFsm = NAS_MSCC_GetCurFsmAddr();
...
)
{
    ...
    if (NAS_MSCC_FSM_SWITCH_ON != enFsmId)
    {
        ...
        /* 通知IMSA Service Change Ind, 经与IMSA讨论，此处需要携带真流程指示，以便IMSA在NORMAL SERVICE
           场景下能够发起注册流程 */
...
/*****************************************************************************
 函 数 名  : NAS_MSCC_RcvMmcPsRegCnRsltInd_PreProc
 功能描述  : 收到MMC的ID_MMC_MSCC_PS_REG_CN_RSLT_IND信息
 输入参数  : ulEventType
             pstMsg
