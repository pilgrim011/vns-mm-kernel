...
VOS_VOID NAS_MMC_SndMsccCurrPsRegCnRsltInd(
    NAS_MSCC_PIF_PS_REG_PROC_ENUM_UINT32                     enPsRegCnRslt
)
{
    MMC_MSCC_PS_REG_CN_RSLT_IND_STRU      *pstMsg = VOS_NULL_PTR;

    /* apply for memory */
    pstMsg = (MMC_MSCC_PS_REG_CN_RSLT_IND_STRU *)PS_ALLOC_MSG(WUEPS_PID_MMC,
                          sizeof(MMC_MSCC_PS_REG_CN_RSLT_IND_STRU) - VOS_MSG_HEAD_LENGTH);

    if (VOS_NULL_PTR == pstMsg)
    {
        NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_SndMsccCurrPsRegCnRsltInd:ERROR:Memory Alloc Error for MMC_MSCC_PS_REG_CN_RSLT_IND_STRU ");
        return;
    }

    PS_MEM_SET(((VOS_UINT8*)pstMsg) + VOS_MSG_HEAD_LENGTH, 0, sizeof(MMC_MSCC_PS_REG_CN_RSLT_IND_STRU) - VOS_MSG_HEAD_LENGTH);

    /* fill message head */
    pstMsg->stMsgHeader.ulMsgName         = ID_MMC_MSCC_PS_REG_CN_RSLT_IND;
    pstMsg->stMsgHeader.ulSenderCpuId     = VOS_LOCAL_CPUID;
    pstMsg->stMsgHeader.ulSenderPid       = WUEPS_PID_MMC;
    pstMsg->stMsgHeader.ulReceiverCpuId   = VOS_LOCAL_CPUID;
    pstMsg->stMsgHeader.ulReceiverPid     = UEPS_PID_MSCC;
    ...
    pstMsg->enPsRegCnRslt                 = enPsRegCnRslt;
