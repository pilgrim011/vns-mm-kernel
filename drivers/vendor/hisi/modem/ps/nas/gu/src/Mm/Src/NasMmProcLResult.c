...
)
{
    ...
    /* 如果当前有主叫电话存在，则先回复CC主叫失败，在处理被叫 */
    if (NAS_MML_CSFB_SERVICE_STATUS_MO_NORMAL_CC_EXIST == NAS_MML_GetCsfbServiceStatus())
    {
        NAS_NORMAL_LOG(WUEPS_PID_MM, "NAS_MM_RcvLmmCsPagingInd: CSFB MO CALL exists, clear MM buffer and handle MT paging");

        Mm_SndCcRelInd(g_MmCcEstReq.ulTransactionId,
                        NAS_MMCM_REL_CAUSE_MM_INTER_ERR_CSFB_ALREADY_EXISTS);     /* 通知CC建立失败 */
