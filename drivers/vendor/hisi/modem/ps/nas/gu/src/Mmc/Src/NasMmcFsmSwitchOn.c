...
)
{
    ...
    /* 卡文件读取完成，如果卡没有换，则判断卡是否在别的手机上使用过 */
    ...
    /* 读完卡文件后，根据NV项中用户配置的Forb PLMN信息更新全局变量和卡文件 */
...
/*****************************************************************************
 函 数 名  : NAS_MMC_UpdateForbPlmnInfo_SwitchOn
 功能描述  : 读完卡文件后，根据NV项中用户配置的Forb PLMN信息更新全局变量和卡文件
 输入参数  : 无
 输出参数  : 无
 返 回 值  : 无
 调用函数  :
...
VOS_VOID NAS_MMC_UpdateForbPlmnInfo_SwitchOn( VOS_VOID )
{
    ...
    for (j = 0; j < ulGroupNum; j++)
    {
        ...
        for ( i = 0; i < stNvimCfgForbPlmnInfo.astForbPlmnInfo[j].ucImsiPlmnListNum; i++ )
        {
            ...
            /* 用户配置的IMSI plmn网络是BCCH格式的，如果和当前这张卡的IMSI号段相同，把该组对应的ForbPlmn信息加到forb列表中 */
        ...
        /* 如果找到一组forbPlmn信息时，把forbPlmn存到全局变量 */
...
/*****************************************************************************
 函 数 名  : NAS_MMC_IsSimCardUsedOtherUe_SwitchOn
 功能描述  : 判断卡是否在别的手机上用过
 输入参数  : VOS_VOID
 输出参数  : 无
 返 回 值  : VOS_UINT32
