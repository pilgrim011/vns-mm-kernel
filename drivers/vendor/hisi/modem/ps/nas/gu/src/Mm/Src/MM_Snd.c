...
VOS_VOID NAS_MM_WriteLocaInCard(VOS_VOID)
{
    ...
    VOS_UINT16                          usFileId;
    NAS_USIMM_UPDATEFILE_INFO_STRU      stSetFileInfo;
    NAS_NVIM_CS_LOCI_SIM_FILES_STRU     stCslociFile;

    NAS_MEM_SET_S(&stCslociFile, sizeof(stCslociFile), 0x00, sizeof(stCslociFile));
    ...
    NAS_MEM_CPY_S(&stCslociFile, sizeof(stCslociFile), pucFileContent, NAS_MML_CS_LOCI_SIM_FILE_LEN);

    if (NV_OK != NV_Write(en_NV_Item_Cs_Loci, &stCslociFile, sizeof(stCslociFile)))
    {
        NAS_ERROR_LOG(WUEPS_PID_MM, "NAS_MM_WriteLocaInCard:ERROR: NV_Write Error");
    }
...
VOS_VOID Mm_SndAgentUsimUpdateFileReq( VOS_UINT16 usEfId )
{
    ...
    PS_MEM_SET(&stCslociFile, 0x00, sizeof(stCslociFile));
    ...
    case USIMM_GSM_EFLOCI_ID:
        ...
        if (0 != PS_MEM_CMP(pucFileContent, NAS_MML_GetSimCsLociFileContent(), MM_SIM_FILE_LEN_LOCA_INFO))
        {
            ...
            if (NV_OK != NV_Write(en_NV_Item_Cs_Loci, &stCslociFile, sizeof(stCslociFile)))
            {
                NAS_ERROR_LOG(WUEPS_PID_MM, "Mm_SndAgentUsimUpdateFileReq:ERROR: NV_Write Error");
            }
