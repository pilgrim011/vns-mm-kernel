...
/*******************************************************************************
  ...
  History:
      ...
      4. 日    期   : 2016年3月17日
         作    者   : z00359541
         修改内容   : DTS2016022503901: NV9247开启时，VPLMN下鉴权被拒也忽略此拒绝消息
*******************************************************************************/
VOS_UINT8 Mm_ComMsgAuthRjctRcv ()
{
    ...
    if (pstAuthRejInfo->ucHplmnCsAuthRejCounter < pstAuthRejInfo->ucMaxAuthRejNo)
    {
        if (VOS_TRUE == pstAuthRejInfo->ucIgnoreAuthRejFlg)
        {
            ...
            NAS_NORMAL_LOG1(WUEPS_PID_MM, "Mm_ComMsgAuthRjctRcv: NV 9247 active, ignore AUTH REJ ", pstAuthRejInfo->ucHplmnCsAuthRejCounter);
...
)
{
    ...
    {
        NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MM_RcvMmcLauReq: MM_NET_MODE_I.");
        return;
    }
    ...
    NAS_WARNING_LOG1(WUEPS_PID_MMC, "NAS_MM_RcvMmcLauReq: MM state.", Mm_GetState());
...
)
{
    ...
    if ((MNCC_CALL_STATUS_SETUP_SUCC != pstCallStatusNty->enCallStatus)
     ...
     && (MNCC_CALL_STATUS_LAST_REDIAL != pstCallStatusNty->enCallStatus)
     )
    {
        return VOS_FALSE;
    }
...
)
{
    ...
    /*如果CALL和SS有任意一个在业务中就不要做FR*/
    ucMoSsSetupFlg = NAS_MML_GetMoSsSetupFlg();
    if ( (VOS_TRUE == NAS_MML_GetMoCallSetupFlg())
     && (VOS_FALSE == ucMoSsSetupFlg) )
    {
...
)
{
    ...
    /*如果CALL和SS有任意一个在业务中就不要做FR*/
    ucMoCallSetupFlg = NAS_MML_GetMoCallSetupFlg();
    if ( (VOS_TRUE == NAS_MML_GetMoSsSetupFlg())
     && (VOS_FALSE == ucMoCallSetupFlg) )
    {
