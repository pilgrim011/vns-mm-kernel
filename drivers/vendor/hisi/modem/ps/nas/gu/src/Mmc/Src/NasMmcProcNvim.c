...
/*****************************************************************************
 函 数 名  : NAS_MMC_ReadCsLociNvim
 功能描述  : 从NV中获取CS Loci文件
 输入参数  : 无
 输出参数  : 无
 返 回 值  : 无
 调用函数  :
...
VOS_VOID NAS_MMC_ReadCsLociNvim(VOS_VOID)
{
    ...
    PS_MEM_SET(&stCsLocInfo, 0x00, sizeof(stCsLocInfo));

    if (NV_OK != NV_Read(en_NV_Item_Cs_Loci,
                         &stCsLocInfo, sizeof(stCsLocInfo)))
    {
        NAS_WARNING_LOG(WUEPS_PID_MMC, "NAS_MMC_ReadCsLociNvim():WARNING: en_NV_Item_Cs_Loci Error");

        return;
    }

    /* 更新开关机状态机中的TMSI的内容 */
    NAS_MMC_SetLastTmsi_SwitchOn(stCsLocInfo.aucTmsi);
    
    /* 更新开关机状态机中的LAI的内容 */
    NAS_MMC_SetLastLai_SwitchOn(stCsLocInfo.aucLastLai);
...
VOS_VOID NAS_MMC_ReadPsLociNvim(VOS_VOID)
{
    ...
    /* 更新开关机状态机中保存的PTMSI */
    NAS_MMC_SetLastPtmsi_SwitchOn(stPsLocInfo.aucPsLocInfo);
    ...
    /* 更新开关机状态机中保存的RAI */
    NAS_MMC_SetLastRai_SwitchOn(&stPsLocInfo.aucPsLocInfo[NAS_MML_RAI_POS_EFPSLOCI_FILE]);
...
{
    VOS_UINT32                          *pulPtmsi = VOS_NULL_PTR;
    if (NV_OK != NV_Write(en_NV_Item_Ps_Loci, pstPsLocInfo, sizeof(NAS_NVIM_PS_LOCI_SIM_FILES_STRU)))
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_WritePsLociNvim:ERROR: (VOS_VOID)NV_Write Error");
    }

    pulPtmsi = (VOS_UINT32*)pstPsLocInfo->aucPsLocInfo;

    NAS_INFO_LOG1(WUEPS_PID_MMC, "NAS_MMC_WritePsLociNvim:NV_Write OK", *pulPtmsi);
...
/*****************************************************************************
 函 数 名  : NAS_MMC_WriteCsLociNvim
 功能描述  : 写CS Loci文件
 输入参数  : pstCsLocInfo CS loc信息
             ulSize       CS loc信息的大小
 输出参数  : 无
 返 回 值  : 无
 调用函数  :
...
)
{
    ...
    if (NV_OK != NV_Write(en_NV_Item_Cs_Loci, pstCsLocInfo, sizeof(NAS_NVIM_CS_LOCI_SIM_FILES_STRU)))
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC, "NAS_MMC_WriteCsLociNvim:ERROR: (VOS_VOID)NV_Write Error");
    }

    pulTmsiValue = (VOS_UINT32*)pstCsLocInfo->aucTmsi;

    NAS_INFO_LOG1(WUEPS_PID_MMC, "NAS_MMC_WriteCsLociNvim:NV_Write OK ", *pulTmsiValue);
