...
VOS_UINT32 NAS_EMM_SndAttachCmpSuccProc(VOS_VOID* pMsg)
{
    ...
    /* 只有在EMM_SS_ATTACH_WAIT_RRC_DATA_CNF状态才会处理；
    其他状态是不正确的状态，就不处理，避免影响其他流程 */
    if (EMM_SS_ATTACH_WAIT_RRC_DATA_CNF != NAS_EMM_CUR_SUB_STAT)
    {
        TLPS_PRINT2LAYER_INFO(NAS_EMM_SndAttachCmpSuccProc_ENUM,LNAS_FUNCTION_LABEL1);
