...
IMSA_ERR_LOG_ALM_LEVEL_STRU g_astImsaErrAlmLevelTb[] = {
    ...
    {IMSA_ERR_LOG_ALM_IMS_REG_FAIL_EVENT_REPORT,            IMSA_ERR_LOG_CTRL_LEVEL_CRITICAL},
    {IMSA_ERR_LOG_ALM_IMS_INIT_DEREG_EVENT_REPORT,          IMSA_ERR_LOG_CTRL_LEVEL_CRITICAL},
    {IMSA_ERR_LOG_ALM_IMS_REG_TIMER_EXPIRE_EVENT_REPORT,    IMSA_ERR_LOG_CTRL_LEVEL_CRITICAL},
    {IMSA_ERR_LOG_ALM_IMS_REG_SUCC_EVENT_REPORT,            IMSA_ERR_LOG_CTRL_LEVEL_CRITICAL},
};

IMSA_ERR_LOG_ALM_REPORT_TYPE_STRU   g_astImsaErrAlmUnsoliReportTypeTb[] = {
    {IMSA_ERR_LOG_ALM_IMS_REG_FAIL_EVENT_REPORT,            IMSA_ERR_LOG_ALARM_REPORT},
    {IMSA_ERR_LOG_ALM_IMS_INIT_DEREG_EVENT_REPORT,          IMSA_ERR_LOG_ALARM_REPORT},
    {IMSA_ERR_LOG_ALM_IMS_REG_TIMER_EXPIRE_EVENT_REPORT,    IMSA_ERR_LOG_ALARM_REPORT},
    {IMSA_ERR_LOG_ALM_IMS_REG_SUCC_EVENT_REPORT,            IMSA_ERR_LOG_ALARM_REPORT},
};
...
/*****************************************************************************
Function Name  : IMSA_InitConnAndRegTimeThresholdWithConfig()
Description    : 使用IMSA_COMM_PARA_CONFIG NV中设置的定时器时长, 修改定时器时长
Input          : VOS_VOID
Output         : VOS_VOID
Return Value   : VOS_VOID
...
*****************************************************************************/
VOS_VOID IMSA_InitConnAndRegTimeThresholdWithConfig
(
    IMSA_COMM_PARA_CONFIG_STRU  *pstNvImsaConfig
)
{
    IMSA_ERRORLOG_MANAGER_STRU          *pstImsaErrorlogManage;

    pstImsaErrorlogManage = IMSA_GetErrorlogManagerAddress();

    /* 配置承载建立时间的阈值 */
    if (IMSA_FALSE != pstNvImsaConfig->ulPdnConnTimeThreshold)
    {
        pstImsaErrorlogManage->ulPdnConnTimeThreshold = (pstNvImsaConfig->ulPdnConnTimeThreshold)*1000;
    }
    else
    {
        pstImsaErrorlogManage->ulPdnConnTimeThreshold = IMSA_CONN_TIMER_THRESHOLD;
    }

    /* 配置注册时间的阈值 */
    if (IMSA_FALSE != pstNvImsaConfig->ulRegTimeThreshold)
    {
        pstImsaErrorlogManage->ulRegTimeThreshold = (pstNvImsaConfig->ulRegTimeThreshold)*1000;
    }
    else
    {
        pstImsaErrorlogManage->ulRegTimeThreshold = IMSA_REG_TIMER_THRESHOLD;
    }
...
VOS_VOID IMSA_ReadImsaNvCommParaConfig(VOS_VOID)
{
    ...
    IMSA_CONTROL_MANAGER_STRU          *pstControlManager;
    #if (FEATURE_ON == FEATURE_PTM)
    IMSA_ERRORLOG_MANAGER_STRU         *pstImsaErrorlogManager;
    #endif
    pstControlManager = IMSA_GetControlManagerAddress();
    #if (FEATURE_ON == FEATURE_PTM)
    ...
    if (VOS_OK == ulRslt)
    {
        ...
        if (IMSA_OP_TRUE == stNvImsaConfig.stImsaCtrlBit.bitOpNonSrvccNotRelCall)
        {
            pstControlManager->ucNonSrvccNotRelCallFlag = IMSA_TRUE;
        }
        else
        {
            pstControlManager->ucNonSrvccNotRelCallFlag = IMSA_FALSE;
        }
        if (IMSA_OP_TRUE == stNvImsaConfig.stImsaCtrlBit.bitOpMediaLostNotRelCall)
        {
            pstControlManager->ucMediaLostNotRelCallFlag = IMSA_TRUE;
        }
        else
        {
            pstControlManager->ucMediaLostNotRelCallFlag = IMSA_FALSE;
        }
        #if (FEATURE_ON == FEATURE_PTM)
        IMSA_InitConnAndRegTimeThresholdWithConfig(&stNvImsaConfig);
        #endif
        if (IMSA_OP_TRUE == stNvImsaConfig.ucUseRetryAfterAsDelayTimeFlag)
        {
            pstControlManager->ucUseRetryAfterAsDelayTimeFlag = IMSA_TRUE;
        }
        else
        {
            pstControlManager->ucUseRetryAfterAsDelayTimeFlag = IMSA_FALSE;
        }
    }
    else
    {
        ...
        #if (FEATURE_ON == FEATURE_PTM)
        pstImsaErrorlogManager->ulPdnConnTimeThreshold   = IMSA_CONN_TIMER_THRESHOLD;
        pstImsaErrorlogManager->ulRegTimeThreshold       = IMSA_REG_TIMER_THRESHOLD;
        #endif
    }
...
*****************************************************************************/
VOS_VOID IMSA_RegFailEventReport
(
...
{
    IMSA_ERR_LOG_ALM_IMS_REG_FAIL_EVENT_REPORT_STRU stImsRegFailEvent;
    ...
    TLPS_PRINT2LAYER_INFO(IMSA_RegFailEventReport_ENUM, LNAS_ENTRY);
    ...
    IMSA_MEM_SET_S(&stImsRegFailEvent, ulLength, 0x00, ulLength);

    IMSA_WRITE_ERRLOG_HEADER_INFO(&stImsRegFailEvent.stHeader,
    ...
    stImsRegFailEvent.enImsRegFailReason =  IMSA_OM_IMS_REG_FAIL_REASON_IMS_REG_FAIL;
    stImsRegFailEvent.stImsRegFailReason.enRegFailReason = enRegFailReason;
    stImsRegFailEvent.stImsRegFailReason.usSipStatusCode = usSipStatusCode;
    stImsRegFailEvent.stImsRegFailReason.ucPermForbFlag  = ucPermForbFlag;
    stImsRegFailEvent.stImsRegFailReason.ucTermForbFlag  = ucTermForbFlag;

    /* 填充当前使用的P-CSCF地址对 */
    if (VOS_TRUE == IMSA_RegAddrPairMgrGetCurrent(IMSA_REG_TYPE_NORMAL, aucUeAddr, aucPcscfAddr))
    {
        IMSA_MEM_CPY_S( stImsRegFailEvent.stImsRegFailReason.aucPcscfAddr,
                        IMSA_IPV6_ADDR_STRING_LEN+1,
                        aucPcscfAddr,
                        IMSA_IPV6_ADDR_STRING_LEN+1);
    }

    IMSA_SndAcpuOmFaultErrLogInd(   &stImsRegFailEvent,
                                    ulLength,
                                    IMSA_ERR_LOG_ALM_IMS_REG_FAIL_EVENT_REPORT);
...
/*****************************************************************************
 函 数 名  : IMSA_GetErrLogUnsoliReportType
 功能描述  : 获取ERR LOG主动上报类型
 输入参数  : IMSA_ERR_LOG_ALM_ID_ENUM_UINT16 enAlmId
 输出参数  : 无
 返 回 值  : IMSA_ERR_LOG_UNSOLI_REPORT_TYPE_ENUM_UINT16
 调用函数  :
 被调函数  :
...
*****************************************************************************/
IMSA_ERR_LOG_UNSOLI_REPORT_TYPE_ENUM_UINT16 IMSA_GetErrLogUnsoliReportType(IMSA_ERR_LOG_ALM_ID_ENUM_UINT16 enAlmId)
{
    VOS_UINT16                          usTableLen;
    VOS_UINT16                          usStep;

    /* 获取Tab表长度 */
    usTableLen = sizeof(g_astImsaErrAlmUnsoliReportTypeTb)/sizeof(g_astImsaErrAlmUnsoliReportTypeTb[0]);

    /* 查表返回对应Alm ID的log等级 */
    for (usStep = 0; usStep < usTableLen; usStep++)
    {
        if (g_astImsaErrAlmUnsoliReportTypeTb[usStep].enAlmID == enAlmId)
        {
            return g_astImsaErrAlmUnsoliReportTypeTb[usStep].enReportType;
        }
    }

    /* 未查到，返回未定义等级 */
    return IMSA_ERR_LOG_REPORT_TYPE_BUTT;
}

/*****************************************************************************
Function Name   : IMSA_SndAcpuOmFaultErrLogInd
...
History         :
 1. wangchen 00209181   2016-03-29  Draft Enact
*****************************************************************************/
VOS_VOID IMSA_SndAcpuOmFaultErrLogInd
(
    VOS_VOID                           *pData,
...
)
{
    ...
    IMSA_ERR_LOG_UNSOLI_REPORT_TYPE_ENUM_UINT16             enReportType;

    TLPS_PRINT2LAYER_INFO(IMSA_SndAcpuOmFaultErrLogInd_ENUM, LNAS_ENTRY);

    /* 申请消息结构内存 */
    /*lint -e961*/
    ulMsgLen     = sizeof(OM_FAULT_ERR_LOG_IND_STRU) - 4 + ulDataLen;
    /*lint +e961*/
    pstFaultRptInd = (VOS_VOID*)IMSA_ALLOC_MSG(ulMsgLen);
    ...
    pstFaultRptInd->ulSenderPid       = PS_PID_IMSA;
    pstFaultRptInd->ulMsgName         = ID_OM_FAULT_ERR_LOG_IND;

    enReportType = IMSA_GetErrLogUnsoliReportType(usAlmId);

    if (IMSA_ERR_LOG_FAULT_REPORT == enReportType)
    {
        pstFaultRptInd->ulMsgType     = OM_ERR_LOG_MSG_FAULT_REPORT;
    }
    else if (IMSA_ERR_LOG_ALARM_REPORT == enReportType)
    {
    ...
    IMSA_MEM_CPY_S((VOS_UINT8 *)ulContentAddr, ulDataLen, pData, ulDataLen);

    /* 发送消息到ACPU OM模块 */
    IMSA_Send_Msg(pstFaultRptInd);
...
/*****************************************************************************
 Function Name  : IMSA_SmsSendIntraResultAction
 Description    : 短信失败后，给Service模块发送的后续操作指示
...
*****************************************************************************/
VOS_UINT32 IMSA_SmsSendIntraResultAction(IMSA_RESULT_ACTION_ENUM_UINT32 enAction)
{
    IMSA_SMS_RESULT_ACTION_IND_STRU stAction = {0};

    IMSA_INFO_LOG("IMSA_SmsSendIntraResultAction is entered!");
    TLPS_PRINT2LAYER_INFO(IMSA_SmsSendIntraResultAction_ENUM, LNAS_ENTRY);

    stAction.enResultAction = enAction;

    return IMSA_RegSendIntraMsg(ID_IMSA_SMS_RESULT_ACTION_IND, (VOS_UINT8 *)&stAction, sizeof(IMSA_SMS_RESULT_ACTION_IND_STRU));
}
