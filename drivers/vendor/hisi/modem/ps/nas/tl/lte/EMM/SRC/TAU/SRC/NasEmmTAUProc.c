...
/*****************************************************************************
 ...
 Description     : 判断是否是VoiceDomain修改触发的TAU是否要携带Active Flag
 Input           :
 Output          : None
 Return          : NAS_EMM_YES:需要置上, NAS_EMM_NO:不需要置上
...
VOS_UINT32  NAS_EMM_IsVoiceDomainChangeSetActFlag()
{
    ...
    /* 若当前TAU发起原因为Domain Change, 且当前为IDLE, 则设上ACT Flag, 防止TAU完成之后网络
       释放导致之后可能上报的IMS去注册/IMS信令承载去激活/信令承载激活消息发送到网侧慢问题 */
