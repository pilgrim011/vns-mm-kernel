...
VOS_VOID    NAS_EMM_ProcRejCauseValOther(NAS_EMM_CN_CAUSE_ENUM_UINT8 ucRejCauseVal)
{
    ...
    else if(NAS_LMM_CAUSE_ESM_FAILURE == ucRejCauseVal)
    {
        ...
        {
            /* add by q00355475: ATTACH被#19原因值拒绝,原来的操作是ATTACH尝试次数直接设为5次,协议中5.5.1.2.6
                 中描述可以但非必须设为5次,GCF测试用例9.2.1.1.30匹配尝试5次ATTACH或者尝试1次ATTACH,此处为解决
                 问题单DTS2016032600075修改为尝试5次ATTACH
               */
            NAS_EMM_GLO_AD_GetAttAtmpCnt() ++;
        }
    }
