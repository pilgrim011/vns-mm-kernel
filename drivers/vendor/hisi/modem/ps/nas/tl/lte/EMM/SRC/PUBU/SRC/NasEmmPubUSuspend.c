...
/*****************************************************************************
 Function Name   : NAS_EMM_CheckGprsUpStateAndRaiChangeAndTin
 Description     : 判断GPRS的UPDATE STATE是否为GU1且RAI没有变更,以及TIN值是否为GUTI.
 Input           : None
 Output          : None
 Return          : VOS_UINT32
 ...
 History         :
    1.leili 00132387      2012-6-5  Draft Enact
    2.jiqiang 00271914    2016-4-9  Modified

*****************************************************************************/
VOS_UINT32  NAS_EMM_CheckGprsUpStateAndRaiChangeAndTin( MsgBlock * pMsg  )
{
    MMC_LMM_ACTION_RESULT_REQ_STRU     *pMmcActResult = VOS_NULL_PTR;
    ...
    if(((NAS_EMM_PUBU_BIT_SLCT == pMmcActResult->bitOpGprsUpdateStatus)
        ...
        || (MMC_LMM_TIN_GUTI == NAS_EMM_GetTinType()))
    {
        return NAS_EMM_SUCC;
    }
...
VOS_VOID  NAS_EMM_ProcGuRauAttemptCounter (MsgBlock * pMsg )
{
     ...
     &&(5 > pMmcActResult->ulAttemptCount))
    {
        /* 如果GPRS update status不为GU1，或者RAI发生变更，则将EPS update status
           改为EU2 */
        if (NAS_EMM_SUCC == NAS_EMM_CheckGprsUpStateAndRaiChangeAndTin(pMsg))
        {
