...
IPSEC_GET_IPPKT_CALLBACK_S       g_stIpsCcoreIpsecHook =
{
    IPS_MNTN_CCORE_IpsecLayerDownCapCB,
    VOS_NULL_PTR,
    VOS_NULL_PTR,
    VOS_NULL_PTR
};

IPSEC6_GET_IPPKT_CALLBACK_S      g_stIpsCcoreIpsec6Hook =
{
    IPS_MNTN_CCORE_IpsecLayerDownCapCB,
    VOS_NULL_PTR,
    VOS_NULL_PTR,
...
IPS_MNTN_CCORE_HOOK_MASK_OPS_STRU             g_stIpsCcoreHookMaskOpt[] =
{
    ...
    },
    {
        ...
        PP_INPUT_TYPE,
        0,
        IPS_MNTN_CCORE_Ip6LayerUpCapCB,
    ...
    },
    {
        ...
        PP_IFOUTPUT_TYPE,
        0,
        IPS_MNTN_CCORE_Ip6LayerDownCapCB,
        (pfRegHook)TCPIP_IP_PP6_Intermediate_Hook_Register,
        VOS_NULL_PTR
    },
};
...
/*****************************************************************************
 函 数 名  : IPS_MNTN_CCORE_IP6IsEspPkt
 功能描述  : 用于判断IPv6报文是否是ESP报文
 输入参数  : pMBuf   报文数据
 输出参数  : 无
 返 回 值  : VOS_UINT32
 调用函数  :
...
*****************************************************************************/
VOS_UINT32 IPS_MNTN_CCORE_IP6IsEspPkt(MBUF_S *pMBuf)
{
    IP6_S                              *pstIP6;
    UCHAR                               ucNxtHdr;
    VOS_UINT32                          ulPktLen;
    VOS_UINT32                          ulOffsetLen;
    UCHAR                              *pucPkt      = VOS_NULL_PTR;

    pstIP6 = MBUF_MTOD(pMBuf, IP6_S *);
    if ( VOS_NULL_PTR == pstIP6 )
    {
        TTF_LOG(UEPS_PID_IPS_CCORE, DIAG_MODE_COMM, PS_PRINT_ERROR,
            "MBUF_MTOD(pMBuf, IP6_S *) is NULL!!!");
        return PS_FALSE;
    }

    ulPktLen    = pMBuf->ulTotalDataLength;
    ulOffsetLen = IPV6_HEAD_NORMAL_LEN;

    /* 获取IPV6基本头中的Next header */
    ucNxtHdr    = pstIP6->ip6_ctlun.ip6_un1.uc_ip6_nxt;
    pucPkt      = MBUF_MTOD(pMBuf, UCHAR *);

    while ((ulOffsetLen + 2) < ulPktLen)
    {
        if (IPPROTO_ESP == ucNxtHdr)
        {
            return PS_TRUE;
        }

        /* 获取IPV6扩展头类型continue */
        if ((IPPROTO_HOPOPTS == ucNxtHdr) ||
            (IPPROTO_ROUTING == ucNxtHdr) ||
            (IPPROTO_DSTOPTS == ucNxtHdr) ||
            (IPPROTO_AH == ucNxtHdr))
        {
            ucNxtHdr = pucPkt[ulOffsetLen];
            ulOffsetLen += ((pucPkt[ulOffsetLen + 1] + 1) << 3);
            continue;
        }

        if (IPPROTO_FRAGMENT == ucNxtHdr)
        {
            ucNxtHdr = pucPkt[ulOffsetLen];
            ulOffsetLen += 8;
            continue;
...
VOS_UINT32 IPS_MNTN_CCORE_IpLayerUpCapCB(MBUF_S *pMBuf)
{
    ...
    pstIP = MBUF_MTOD(pMBuf, IP_S *);
    if ( VOS_NULL_PTR == pstIP )
    {
        TTF_LOG(UEPS_PID_IPS_CCORE, DIAG_MODE_COMM, PS_PRINT_ERROR,
            "MBUF_MTOD(pMBuf, IP_S *) is NULL!!!");
        return PS_FAIL;
    }
    ...
    /* 加密报文处理 */
    if (IPPROTO_ESP == pstIP->ip_chPr)
    {
        /* 配置了勾密文 */
        if (WTTF_TRACE_MSG_YES == g_stIpsCcoreTraceMsgCfg.stVispLogConfig.enVISPEncryptPktCapConfig)
        {
            IPS_MNTN_CCORE_PktInfo((VOS_UINT8 *)pMBuf, (VOS_UINT16)ulPktLen, ID_IPS_CCORE_TRACE_INPUT_DATA_INFO);
        }
    }
    else
    {
        /* 明文 */
        if (VOS_OK == TCPIP_IPSEC_IsAllowedCapturePkt(pMBuf))
        {
            IPS_MNTN_CCORE_PktInfo((VOS_UINT8 *)pMBuf, (VOS_UINT16)ulPktLen, ID_IPS_CCORE_TRACE_INPUT_DATA_INFO);
        }
        else
        {
            /* 密文 但是配置了勾明文 */
            if (WTTF_TRACE_MSG_YES == g_stIpsCcoreTraceMsgCfg.stVispLogConfig.enVISPUnEncryptPktCapConfig)
            {
                IPS_MNTN_CCORE_PktInfo((VOS_UINT8 *)pMBuf, (VOS_UINT16)ulPktLen, ID_IPS_CCORE_TRACE_INPUT_DATA_INFO);
            }
        }
...
/*****************************************************************************
 函 数 名  : IPS_MNTN_CCORE_Ip6LayerUpCapCB
 功能描述  : RTOSck平台IP6层上行抓包回调函数
 输入参数  : pMBuf   报文数据
 输出参数  : 无
 返 回 值  : VOS_UINT32
 调用函数  :
...
*****************************************************************************/
VOS_UINT32 IPS_MNTN_CCORE_Ip6LayerUpCapCB(MBUF_S *pMBuf)
{
    VOS_UINT32                          ulPktLen;
    ...
    /* 加密报文处理 */
    if (PS_TRUE == IPS_MNTN_CCORE_IP6IsEspPkt(pMBuf))
    {
        /* 配置了勾密文 */
        if (WTTF_TRACE_MSG_YES == g_stIpsCcoreTraceMsgCfg.stVispLogConfig.enVISPEncryptPktCapConfig)
        {
...
/*****************************************************************************
 函 数 名  : IPS_MNTN_CCORE_IpsecLayerDownCapCB
 功能描述  : RTOSck平台IP层上行抓包回调函数
 输入参数  : pMBuf   报文数据
 输出参数  : 无
 返 回 值  : VOS_UINT32
...
*****************************************************************************/
ULONG IPS_MNTN_CCORE_IpsecLayerDownCapCB(MBUF_S *pMBuf)
{
    if ( VOS_NULL_PTR == pMBuf )
    {
        TTF_LOG(UEPS_PID_IPS_CCORE, DIAG_MODE_COMM, PS_PRINT_ERROR,
    ...
    if (WTTF_TRACE_MSG_YES == g_stIpsCcoreTraceMsgCfg.stVispLogConfig.enVISPUnEncryptPktCapConfig)
    {
        IPS_MNTN_CCORE_PktInfo((VOS_UINT8 *)pMBuf, (VOS_UINT16)pMBuf->ulTotalDataLength, ID_IPS_CCORE_TRACE_OUTPUT_DATA_INFO);
    }
...
VOS_UINT32 IPS_MNTN_CCORE_IpLayerDownCapCB(MBUF_S *pMBuf)
{
    ...
    if (IPPROTO_ESP == pstIP->ip_chPr)
    {
        ...
        if (WTTF_TRACE_MSG_YES == g_stIpsCcoreTraceMsgCfg.stVispLogConfig.enVISPEncryptPktCapConfig)
        {
            IPS_MNTN_CCORE_PktInfo((VOS_UINT8 *)pMBuf, (VOS_UINT16)ulPktLen, ID_IPS_CCORE_TRACE_OUTPUT_DATA_INFO);
        }
    }
    else
    {
...
/*****************************************************************************
 函 数 名  : IPS_MNTN_CCORE_Ip6LayerDownCapCB
 功能描述  : RTOSck平台IP层下行抓包回调函数
 输入参数  : pMBuf   报文数据
 输出参数  : 无
 返 回 值  : VOS_UINT32
...
*****************************************************************************/
VOS_UINT32 IPS_MNTN_CCORE_Ip6LayerDownCapCB(MBUF_S *pMBuf)
{
    VOS_UINT32                          ulPktLen;
...
VOS_VOID IPS_MNTN_CCORE_TraceAdvancedCfgReq(VOS_VOID *pMsg)
{
    ...
    (VOS_VOID)TCPIP_IPSEC_GetIPPkt_RegCallbackAPI(&g_stIpsCcoreIpsecHook);
    (VOS_VOID)TCPIP_IPSEC6_GetIPPkt_RegCallbackAPI(&g_stIpsCcoreIpsec6Hook);
...
VOS_UINT32 IPS_MNTN_CCORE_FidInit ( enum VOS_INIT_PHASE_DEFINE ip )
{
    ...
    case    VOS_IP_LOAD_CONFIG:
        ...
        /* 初始化勾包配置 */
        g_stIpsCcoreTraceMsgCfg.stBridgeTraceCfg.ulChoice                   = IPS_MNTN_CCORE_TRACE_NULL_CHOSEN;
        g_stIpsCcoreTraceMsgCfg.stInputTraceCfg.ulChoice                    = IPS_MNTN_CCORE_TRACE_NULL_CHOSEN;
        g_stIpsCcoreTraceMsgCfg.stOutputTraceCfg.ulChoice                   = IPS_MNTN_CCORE_TRACE_NULL_CHOSEN;
        g_stIpsCcoreTraceMsgCfg.stVispLogConfig.enVISPLogOutConfig          = WTTF_TRACE_MSG_NO;
        g_stIpsCcoreTraceMsgCfg.stVispLogConfig.enVISPRunLogLevelConfig     = IPS_MNTN_CCORE_TCPIP_LOG_LEVEL_CRITICAL;
        g_stIpsCcoreTraceMsgCfg.stVispLogConfig.enVISPUnEncryptPktCapConfig = WTTF_TRACE_MSG_NO;
        g_stIpsCcoreTraceMsgCfg.stVispLogConfig.enVISPEncryptPktCapConfig   = WTTF_TRACE_MSG_YES;
