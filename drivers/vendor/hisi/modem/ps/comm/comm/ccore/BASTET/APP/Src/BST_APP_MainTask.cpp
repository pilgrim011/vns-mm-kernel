      ...
      BST_SRV_CAsRcver( BST_SRV_GetSysMsgHandle() )
{
    ...
    m_ulBstSupportFlag                  =  BST_SYS_SUPPORT_NONE;
   ...
   *pucNewAddr                          = BST_NULL_PTR;
    ...
    switch ( enInPid )
    {
        ...
        case BST_PID_AP_SLEEP_STATE:
            ...
            /* 转换为亮屏状态，立即上报'代理标志位' */
            if ( m_ulApSleepState == BST_DRV_LS_AP_STATE_WAKEUP )
            {
                pcReporter         = new BST_DSPP_CReport( m_usProcId, m_usTaskId );

                pcReporter->Report( BST_PID_FEATURE_SUPPORT,
                                    BST_OS_SIZEOF( BST_UINT32 ),
                                    (BST_VOID *)&m_ulBstSupportFlag );
                delete pcReporter;
            }
            break;
        ...
        case BST_AS_EVT_L3_RAT_STATE:
            {
                ...
                /* 灭屏状态并且‘代理标志位’没改变时，不触发上报 */
                if ( m_ulApSleepState == BST_DRV_LS_AP_STATE_SLEEP
                     && m_ulBstSupportFlag == ulReportErrVal )
                {
                        break;
                }
                m_ulBstSupportFlag = ulReportErrVal;
