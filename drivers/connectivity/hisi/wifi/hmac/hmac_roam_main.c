...
OAL_STATIC oal_uint32  hmac_roam_connect_to_bss(hmac_roam_info_stru *pst_roam_info, oal_void *p_param)
{
    ...
    pst_old_bss->uc_obss_scan_support = mac_vap_get_peer_obss_scan(&pst_hmac_vap->st_vap_base_info);
    pst_old_bss->ul_obss_scanInterval = mac_mib_get_BSSWidthTriggerScanInterval(&pst_hmac_vap->st_vap_base_info);
...
OAL_STATIC oal_uint32  hmac_roam_to_old_bss(hmac_roam_info_stru *pst_roam_info, oal_void *p_param)
{
    ...
    mac_vap_set_peer_obss_scan(&pst_hmac_vap->st_vap_base_info, pst_old_bss->uc_obss_scan_support);
    mac_mib_set_BSSWidthTriggerScanInterval(&pst_hmac_vap->st_vap_base_info, pst_old_bss->ul_obss_scanInterval);
#if (_PRE_MULTI_CORE_MODE_OFFLOAD_DMAC == _PRE_MULTI_CORE_MODE)
    hmac_config_set_obss_scan_param(&pst_hmac_vap->st_vap_base_info);
    hmac_config_sync_peer_obss_scan(&pst_hmac_vap->st_vap_base_info);
#endif
