...
/*****************************************************************************
 函 数 名  : hmac_config_sync_peer_obss_scan
 功能描述  : 整体同步obss scan相关的mib值
 输入参数  : mac_vap_stru *pst_mac_vap
 输出参数  : 无
 返 回 值  :
 ...
 修改历史      :
  1.日    期   : 2016年5月16日
    ...
    修改内容   : 新生成函数
*****************************************************************************/
oal_uint32  hmac_config_sync_peer_obss_scan(mac_vap_stru *pst_mac_vap)
{
    oal_uint32 ul_ret;
    oal_uint8  uc_peer_obss_scan;

    uc_peer_obss_scan = pst_mac_vap->st_cap_flag.bit_peer_obss_scan;

    OAM_INFO_LOG1(pst_mac_vap->uc_device_id, OAM_SF_CFG, "{hmac_config_sync_peer_obss_scan::uc_peer_obss_scan:%u.}", uc_peer_obss_scan);

    ul_ret = hmac_config_send_event(pst_mac_vap, WLAN_CFGID_SET_PERR_OBSS_SCAN,
                OAL_SIZEOF(uc_peer_obss_scan),
                (oal_uint8*)&uc_peer_obss_scan);

    if (OAL_UNLIKELY(OAL_SUCC != ul_ret))
    {
        OAM_WARNING_LOG1(pst_mac_vap->uc_vap_id, OAM_SF_CFG,
            "{hmac_config_sync_peer_obss_scan::hmac_config_send_event failed[%d].}", ul_ret);
    }
